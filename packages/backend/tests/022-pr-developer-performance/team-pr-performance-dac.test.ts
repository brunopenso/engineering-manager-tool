...
describe('team PR performance DAC', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('scopes performance through service for leader actor without userId', async () => {
    ...
    expect(leaderPrPerformanceService.getLeaderTeamPrPerformance).toHaveBeenCalledWith(
      'leader-1',
      expect.objectContaining({ startDate: '2026-06-14', endDate: '2026-08-12' }),
    );
  ...
  it('asserts subtree membership before scoped fetch', async () => {
    const app = buildTeamPrPerformanceTestApp({
      userId: 'leader-1',
      roles: [USER_ROLE_TYPES.COLLABORATOR, USER_ROLE_TYPES.LEADER],
    });
