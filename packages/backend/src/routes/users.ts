...
import {
  getLeaderDeveloperPrDrilldown,
  getLeaderTeamPrPerformance,
} from '../services/leaderPrPerformanceService.js';
...
export async function registerUsersRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{
    ...
    '/users/leader/team-pr-performance/developers/:userId/pull-requests',
    async (request, reply) => {
      const auth = requireAuth(request, reply);
      if (!auth) {
        return {
      ...
      const developerUserId = request.params.userId?.trim();
      const startDate = request.query.startDate?.trim();
      const endDate = request.query.endDate?.trim();

      if (!developerUserId) {
        return validationError(reply, 'userId path parameter is required.');
      }
      ...
      try {
        await assertUserInLeaderSubtree(auth.userId, developerUserId);
      } catch {
        return forbidden(reply);
      }

      try {
        return await getLeaderDeveloperPrDrilldown(auth.userId, developerUserId, {
          startDate,
          endDate,
        });
      } catch (error) {
