...
const FIRST_NAMES = [
  ...
  'Indigo', 'Jules', 'Kennedy', 'Logan', 'Marley', 'Nico', 'Oakley', 'Peyton',
  'Quinn', 'Reese', 'Skyler', 'Tatum', 'Uma', 'Vale', 'Winter', 'Xander',
  'Yael', 'Zion', 'Adrian', 'Bianca', 'Caleb', 'Delia', 'Elena', 'Felix',
  'Gina', 'Hugo', 'Isla', 'Jonah', 'Keira', 'Leo', 'Maya', 'Nolan',
  'Opal', 'Priya', 'Remy', 'Sienna', 'Theo', 'Uma', 'Vera', 'Wes',
  'Xena', 'Yara', 'Zane', 'Amir', 'Bella', 'Cole', 'Diana', 'Ethan',
  'Fiona', 'Gavin', 'Hannah', 'Ivan', 'Jade',
];

const LAST_NAMES = [
  'Adams', 'Bennett', 'Clark', 'Diaz', 'Edwards', 'Foster', 'Garcia', 'Hayes',
  'Ingram', 'Jensen', 'Khan', 'Lopez', 'Morales', 'Nguyen', 'Owens', 'Price',
  'Ramirez', 'Singh', 'Torres', 'Underwood', 'Vasquez', 'Walsh', 'Xu', 'Young',
  'Zimmerman', 'Baker', 'Cooper', 'Dunn', 'Evans', 'Flores', 'Grant', 'Hughes',
];
...
async function findSeedUser(manager: EntityManager, email: string): Promise<User> {
  ...
  if (!user) {
    throw new Error(
      `Seed user ${email} not found. Run 002-sr-manager-reports seed first.`,
    );
  }
