...
export default function LeaderTeamPrPerformancePage() {
  ...
  const loadPerformance = useCallback(async () => {
    ...
    try {
      const data = await fetchTeamPrPerformance(accessToken, { startDate, endDate });
  ...
  }, [accessToken, dateRangeIsValid, endDate, startDate, t]);
  ...
  }, [accessToken, isLoadingHierarchy, loadPerformance, user]);
  ...
  return (
    <Container maxWidth="lg" sx={{ py: { xs: 2, md: 3 } }}>
      <Stack spacing={3}>
        ...
        {isLoadingHierarchy || isLoadingPerformance ? (
          <Box sx={{ display: 'flex', justifyContent: 'center', py: 4 }}>
            <CircularProgress aria-label={t('teamPrPerformance.loadingPerformance')} />
          </Box>
        ) : null}

        {!isLoadingHierarchy && !isLoadingPerformance && hasTeamMembers && performance ? (
